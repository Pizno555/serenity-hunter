...
## v2.0.0 guard additions (appended, original content above untouched)

### Citation density management

For deep scans with 25+ sources:

- inline citations: max 3 per paragraph;
- full source list goes in a compact table at the end of the answer, not scattered inline;
- prose leads with the judgment; citations support it, not the other way around;
- if a paragraph has more than 3 citations, move the extras to the end-of-answer source table and reference by number.

Source table format:

| # | Source type | What it supports | Strength |
|---|---|---|---|

### Evidence-label discipline in output

When writing the final answer:

- every evidence label (Strong / Medium / Weak / Needs checking) must match the strict mapping in `references/evidence-ladder.md`;
- do not upgrade a source's strength to make the answer sound more convincing;
- if uncertain about strength, default to the lower level;
- "Needs checking" is a valid label, not a failure. Use it honestly.
